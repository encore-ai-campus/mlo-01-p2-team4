...
해당 ERD는 MVP 완성형 기준이며 현재는 Silver 계층 데이터 표준화·정규화 및 직원–조직 통합 참조 데이터 구축 완료 상태
...
| 이름 | 회고 |
|---|---|
| 안길찬 | 크롤링 기능 구현에서 시작해 버전 관리, Bronze 원본 보존, MongoDB Atlas 증분 적재까지 확장하며 운영 가능한 데이터 파이프라인을 설계하는 경험을 쌓았고, 설계를 진행하는 과정에서 시간비용, 공수에 대한 생각을 할 수 있었던 프로젝트였습니다. |
...
| 이형인 | 데이터 표준화 및 서비스 기획 경험을 쌓을 수 있어 유익했고 개발이 아니더라도 컴퓨터 용어에 대한 상식이 부족했는데 프로젝트를 통해 많이 배울 수 있었습니다. 부족한게 많았는데 옆에서 도와주신 팀원분들께도 감사드리며 많은 것을 느낀 프로젝트였습니다. |
