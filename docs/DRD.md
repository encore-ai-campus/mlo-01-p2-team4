...
| 요청 제공본 standard-terms.csv | 표준 용어·물리명·원천 컬럼 매핑·필드별 nullability |
| 요청 제공본 standard-words.csv | 표준 단어와 조합 규칙 |

비즈니스 범위와 규정은 BRD 및 Appendix를 우선하고, 실행·릴리스 기준은 PRD를 따른다. 역공학 보고서의 PK·FK·카디널리티는 데이터 패턴에서 도출한 후보로만 취급한다.

이번 요청에 붙은 표준자료는 AREA 기반 용어와 DATETIME_ISO, ACTIVE·INACTIVE 도메인을 정의한다. 현재 checkout의 reference/ 아래 표준 파일에는 DIVISION 기반의 다른 초안 정의가 있으므로 동일 버전으로 간주하지 않는다. 본 DRD를 실행 계약으로 사용할 때는 요청 제공본을 기준으로 reference/ 스냅샷의 버전·checksum을 먼저 일치시켜야 하며, 이 문서는 해당 파일을 수정하지 않는다.
